...
import { Plus, Pencil, Trash2, Building2, Loader2, X, BookOpen } from 'lucide-react'
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <h3 className="font-display font-bold text-lg text-slate-800">{isEdit ? 'Edit Department' : 'Add Department'}</h3>
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Field label="Department Name" req>
              <input className={inputCls} value={form.name} onChange={e => set('name', e.target.value)} placeholder="Information Technology" />
            </Field>
            <Field label="Code" req>
              <input className={inputCls} value={form.code} onChange={e => set('code', e.target.value.toUpperCase())} placeholder="IT" maxLength={10} />
            </Field>
          </div>
          <Field label="Description">
            <textarea className={inputCls} rows={3} value={form.description} onChange={e => set('description', e.target.value)} placeholder="Department description..." />
          </Field>
          <Field label="Head of Department">
            <select className={inputCls} value={form.head_of_department} onChange={e => set('head_of_department', e.target.value)}>
        ...
        <div className="p-6 pt-0 flex gap-3">
          ...
          <button onClick={handleSubmit} disabled={loading}
            ...
            {loading ? <><Loader2 size={16} className="animate-spin" /> Saving...</> : isEdit ? 'Save Changes' : 'Create'}
          </button>
        </div>
      </div>
    </div>
...
export default function DepartmentsPage() {
  ...
  const [editDept, setEditDept] = useState(null)   // null = closed, {} = create, dept = edit
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {departments.map(dept => (
            <div key={dept.id} className="bg-white rounded-2xl border border-slate-200 p-6 hover:shadow-md transition-shadow group">
              {/* Top */}
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="bg-slate-100 text-slate-600 text-xs font-bold px-2 py-0.5 rounded-lg">{dept.code}</span>
                </div>
                {dept.description && <p className="text-slate-400 text-sm mb-3 line-clamp-2">{dept.description}</p>}
