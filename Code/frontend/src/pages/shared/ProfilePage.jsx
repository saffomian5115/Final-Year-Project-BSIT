...
import {
  ...
  Building2, BookOpen, Hash, KeyRound, Eye, EyeOff
} from 'lucide-react'
...
function ChangePasswordModal({ onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    if (form.new_password !== form.confirm_password) {
      toast.error('New password aur confirm password match nahi karte'); return
    }
    if (form.current_password === form.new_password) {
      toast.error('New password old password se alag hona chahiye'); return
    }
      ...
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-display font-bold text-lg text-slate-800">Change Password</h3>
          </div>
...
export default function ProfilePage() {
  ...
  const ROLE_BADGE = { admin: 'bg-purple-100 text-purple-700', teacher: 'bg-emerald-100 text-emerald-700', student: 'bg-blue-100 text-blue-700' }
        ...
        <div className="px-6 pb-6">
          <div className="flex items-end justify-between -mt-10">
            ...
            <div className="relative">
              {avatarUrl ? (
                ...
                <div className={`w-20 h-20 ${ROLE_COLOR[profile?.role]} rounded-2xl border-4 border-white shadow-lg flex items-center justify-center text-white font-bold text-2xl`}>
          ...
          <div className="mt-3">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-xl font-display font-bold text-slate-800">{profile?.full_name}</h1>
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${ROLE_BADGE[profile?.role]}`}>{profile?.role}</span>
              {profile?.is_active && (
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        ...
        {editing ? (
          ...
          <div>
            ...
            <InfoRow icon={MapPin}    label="Address"        value={profile?.current_address} />
            <InfoRow icon={Building2} label="Designation"    value={profile?.designation} />
            ...
            <InfoRow icon={Calendar}  label="Joined"         value={profile?.joining_date} />
            <InfoRow icon={Shield}    label="Last Login"     value={profile?.last_login ? new Date(profile.last_login).toLocaleString() : null} />
          </div>
        )}
      </div>
