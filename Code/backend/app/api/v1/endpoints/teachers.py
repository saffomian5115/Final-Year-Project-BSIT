...
from app.schemas.user import TeacherUpdateRequest
...
def get_teachers(
    ...
    teachers, total = UserService.get_teachers(db, page, per_page)
...
def update_teacher(
    ...
    teacher, error = UserService.update_teacher(db, teacher_id, request)
...
def update_teacher(
    teacher_id: int,
    request: TeacherUpdateRequest,   # dict ki jagah proper schema
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)
):
